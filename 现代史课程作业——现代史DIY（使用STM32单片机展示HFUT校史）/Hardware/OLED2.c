...
/**
  ...
  */
uint8_t OLED_DisplayBuf2[8][128];
...
/**
  ...
  */
void OLED_W_SCL2(uint8_t BitValue)
{
...
/**
  ...
  */
void OLED_W_SDA2(uint8_t BitValue)
{
...
/**
  ...
  * 返 回 值：无
  */
void OLED_I2C_Start2(void)
{
	OLED_W_SDA2(1);		//释放SDA，确保SDA为高电平
	OLED_W_SCL2(1);		//释放SCL，确保SCL为高电平
	OLED_W_SDA2(0);		//在SCL高电平期间，拉低SDA，产生起始信号
	OLED_W_SCL2(0);		//起始后把SCL也拉低，即为了占用总线，也为了方便总线时序的拼接
}
...
/**
  ...
  * 返 回 值：无
  */
void OLED_I2C_Stop2(void)
{
	OLED_W_SDA2(0);		//拉低SDA，确保SDA为低电平
	OLED_W_SCL2(1);		//释放SCL，使SCL呈现高电平
	OLED_W_SDA2(1);		//在SCL高电平期间，释放SDA，产生终止信号
}
...
/**
  ...
  * 返 回 值：无
  */
void OLED_I2C_SendByte2(uint8_t Byte)
{
	uint8_t i;
	...
	for (i = 0; i < 8; i++)
	{
		...
		/*两个!的作用是，让所有非零的值变为1*/
		OLED_W_SDA2(!!(Byte & (0x80 >> i)));
		OLED_W_SCL2(1);	//释放SCL，从机在SCL高电平期间读取SDA
		OLED_W_SCL2(0);	//拉低SCL，主机开始发送下一位数据
	}
	
	OLED_W_SCL2(1);		//额外的一个时钟，不处理应答信号
	OLED_W_SCL2(0);
}
...
/**
  ...
  * 返 回 值：无
  */
void OLED_WriteCommand2(uint8_t Command)
{
	OLED_I2C_Start2();				//I2C起始
	OLED_I2C_SendByte2(0x78);		//发送OLED的I2C从机地址
	OLED_I2C_SendByte2(0x00);		//控制字节，给0x00，表示即将写命令
	OLED_I2C_SendByte2(Command);		//写入指定的命令
	OLED_I2C_Stop2();				//I2C终止
}
...
/**
  ...
  * 返 回 值：无
  */
void OLED_WriteData2(uint8_t *Data, uint8_t Count)
{
	uint8_t i;
	
	OLED_I2C_Start2();				//I2C起始
	OLED_I2C_SendByte2(0x78);		//发送OLED的I2C从机地址
	OLED_I2C_SendByte2(0x40);		//控制字节，给0x40，表示即将写数据
	/*循环Count次，进行连续的数据写入*/
	for (i = 0; i < Count; i ++)
	{
		OLED_I2C_SendByte2(Data[i]);	//依次发送Data的每一个数据
	}
	OLED_I2C_Stop2();				//I2C终止
}
...
{
	OLED_GPIO_Init2();			//先调用底层的端口初始化
	
	/*写入一系列的命令，对OLED进行初始化配置*/
	OLED_WriteCommand2(0xAE);	//设置显示开启/关闭，0xAE关闭，0xAF开启
	
	OLED_WriteCommand2(0xD5);	//设置显示时钟分频比/振荡器频率
	OLED_WriteCommand2(0x80);	//0x00~0xFF
	
	OLED_WriteCommand2(0xA8);	//设置多路复用率
	OLED_WriteCommand2(0x3F);	//0x0E~0x3F
	
	OLED_WriteCommand2(0xD3);	//设置显示偏移
	OLED_WriteCommand2(0x00);	//0x00~0x7F
	
	OLED_WriteCommand2(0x40);	//设置显示开始行，0x40~0x7F
	...
	OLED_WriteCommand2(0x81);	//设置对比度
	OLED_WriteCommand2(0xCF);	//0x00~0xFF
	...
	OLED_WriteCommand2(0xA6);	//设置正常/反色显示，0xA6正常，0xA7反色
	...
	OLED_Clear2();				//清空显存数组
	OLED_Update2();				//更新显示，清屏，防止初始化后未显示内容时花屏
}
...
/**
  ...
  */
void OLED_SetCursor2(uint8_t Page, uint8_t X)
{
...
//	X += 2;
	...
	/*通过指令设置页地址和列地址*/
	OLED_WriteCommand2(0xB0 | Page);					//设置页位置
	OLED_WriteCommand2(0x10 | ((X & 0xF0) >> 4));	//设置X位置高4位
	OLED_WriteCommand2(0x00 | (X & 0x0F));			//设置X位置低4位
}
...
/**
  ...
  */
uint8_t OLED_pnpoly2(uint8_t nvert, int16_t *vertx, int16_t *verty, int16_t testx, int16_t testy)
{
	int16_t i, j, c = 0;
...
/**
  ...
  */
uint8_t OLED_IsInAngle2(int16_t X, int16_t Y, int16_t StartAngle, int16_t EndAngle)
{
	int16_t PointAngle;
...
void OLED_Update2(void)
{
	...
	for (j = 0; j < 8; j ++)
	{
		...
		OLED_WriteData2(OLED_DisplayBuf2[j], 128);
	}
}
...
void OLED_UpdateArea2(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	for (j = Page; j < Page1; j ++)
	{
		if (X >= 0 && X <= 127 && j >= 0 && j <= 7)		//超出屏幕的内容不显示
		{
			...
			OLED_WriteData2(&OLED_DisplayBuf2[j][X], Width);
		}
	}
}
...
void OLED_Clear2(void)
{
	...
	for (j = 0; j < 8; j ++)				//遍历8页
	{
		...
		{
			OLED_DisplayBuf2[j][i] = 0x00;	//将显存数组数据全部清零
		}
	}
}
...
void OLED_ClearArea2(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	for (j = Y; j < Y + Height; j ++)		//遍历指定页
	{
		for (i = X; i < X + Width; i ++)	//遍历指定列
		{
			...
			{
				OLED_DisplayBuf2[j / 8][i] &= ~(0x01 << (j % 8));	//将显存数组指定数据清零
			}
		}
	}
}
...
void OLED_Reverse2(void)
{
	...
	for (j = 0; j < 8; j ++)				//遍历8页
	{
		for (i = 0; i < 128; i ++)			//遍历128列
		{
			OLED_DisplayBuf2[j][i] ^= 0xFF;	//将显存数组数据全部取反
		}
	}
}
...
void OLED_ReverseArea2(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	for (j = Y; j < Y + Height; j ++)		//遍历指定页
	{
		for (i = X; i < X + Width; i ++)	//遍历指定列
		{
			if (i >= 0 && i <= 127 && j >=0 && j <= 63)			//超出屏幕的内容不显示
			{
				OLED_DisplayBuf2[j / 8][i] ^= 0x01 << (j % 8);	//将显存数组指定数据取反
			}
		}
	}
}
...
void OLED_ShowChar2(int16_t X, int16_t Y, char Char, uint8_t FontSize)
{
	if (FontSize == OLED_8X16)		//字体为宽8像素，高16像素
	{
		...
		OLED_ShowImage2(X, Y, 8, 16, OLED_F8x16[Char - ' ']);
	}
	else if(FontSize == OLED_6X8)	//字体为宽6像素，高8像素
	{
		/*将ASCII字模库OLED_F6x8的指定数据以6*8的图像格式显示*/
		OLED_ShowImage2(X, Y, 6, 8, OLED_F6x8[Char - ' ']);
	}
}
...
void OLED_ShowString2(int16_t X, int16_t Y, char *String, uint8_t FontSize)
{
	...
	while (String[i] != '\0')	//遍历字符串
	{
		...
		if (CharLength == 1)	//如果是单字节字符
		{
			...
			OLED_ShowChar2(X + XOffset, Y, SingleChar[0], FontSize);
			XOffset += FontSize;
		}
		else					//否则，即多字节字符
		{
			...
			if (FontSize == OLED_8X16)		//给定字体为8*16点阵
			{
				...
				OLED_ShowImage2(X + XOffset, Y, 16, 16, OLED_CF16x16[pIndex].Data);
				XOffset += 16;
			}
			else if (FontSize == OLED_6X8)	//给定字体为6*8点阵
			{
				/*空间不足，此位置显示'?'*/
				OLED_ShowChar2(X + XOffset, Y, '?', OLED_6X8);
				XOffset += OLED_6X8;
			}
		}
	}
...
void OLED_ShowNum2(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
{
	...
	for (i = 0; i < Length; i++)		//遍历数字的每一位							
	{
		...
		/*+ '0' 可将数字转换为字符格式*/
		OLED_ShowChar2(X + i * FontSize, Y, Number / OLED_Pow2(10, Length - i - 1) % 10 + '0', FontSize);
	}
}
...
void OLED_ShowSignedNum2(int16_t X, int16_t Y, int32_t Number, uint8_t Length, uint8_t FontSize)
{
	...
	if (Number >= 0)						//数字大于等于0
	{
		OLED_ShowChar2(X, Y, '+', FontSize);	//显示+号
	...
	for (i = 0; i < Length; i++)			//遍历数字的每一位								
	{
		...
		/*+ '0' 可将数字转换为字符格式*/
		OLED_ShowChar2(X + (i + 1) * FontSize, Y, Number1 / OLED_Pow2(10, Length - i - 1) % 10 + '0', FontSize);
	}
}
...
void OLED_ShowHexNum2(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
{
	...
	for (i = 0; i < Length; i++)		//遍历数字的每一位
	{
		...
		if (SingleNumber < 10)			//单个数字小于10
		{
			...
			/*+ '0' 可将数字转换为字符格式*/
			OLED_ShowChar2(X + i * FontSize, Y, SingleNumber + '0', FontSize);
		}
		else							//单个数字大于10
		{
			/*调用OLED_ShowChar函数，显示此数字*/
			/*+ 'A' 可将数字转换为从A开始的十六进制字符*/
			OLED_ShowChar2(X + i * FontSize, Y, SingleNumber - 10 + 'A', FontSize);
		}
	}
}
...
void OLED_ShowBinNum2(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
{
	...
	for (i = 0; i < Length; i++)		//遍历数字的每一位	
	{
		...
		/*+ '0' 可将数字转换为字符格式*/
		OLED_ShowChar2(X + i * FontSize, Y, Number / OLED_Pow2(2, Length - i - 1) % 2 + '0', FontSize);
	}
}
...
void OLED_ShowFloatNum2(int16_t X, int16_t Y, double Number, uint8_t IntLength, uint8_t FraLength, uint8_t FontSize)
{
	...
	PowNum = OLED_Pow2(10, FraLength);		//根据指定小数的位数，确定乘数
	FraNum = round(Number * PowNum);		//将小数乘到整数，同时四舍五入，避免显示误差
	IntNum += FraNum / PowNum;				//若四舍五入造成了进位，则需要再加给整数
	
	/*显示整数部分*/
	OLED_ShowNum2(X + FontSize, Y, IntNum, IntLength, FontSize);
	
	/*显示小数点*/
	OLED_ShowChar2(X + (IntLength + 1) * FontSize, Y, '.', FontSize);
	
	/*显示小数部分*/
	OLED_ShowNum2(X + (IntLength + 2) * FontSize, Y, FraNum, FraLength, FontSize);
}
...
void OLED_ShowImage2(int16_t X, int16_t Y, uint8_t Width, uint8_t Height, const uint8_t *Image)
{
	...
	/*将图像所在区域清空*/
	OLED_ClearArea2(X, Y, Width, Height);
	...
	for (j = 0; j < (Height - 1) / 8 + 1; j ++)
	{
		...
		for (i = 0; i < Width; i ++)
		{
			if (X + i >= 0 && X + i <= 127)		//超出屏幕的内容不显示
			{
				...
				{
					/*显示图像在当前页的内容*/
					OLED_DisplayBuf2[Page + j][X + i] |= Image[j * Width + i] << (Shift);
				}
				
				if (Page + j + 1 >= 0 && Page + j + 1 <= 7)		//超出屏幕的内容不显示
				{					
					/*显示图像在下一页的内容*/
					OLED_DisplayBuf2[Page + j + 1][X + i] |= Image[j * Width + i] >> (8 - Shift);
				}
			}
		}
	}
...
void OLED_Printf2(int16_t X, int16_t Y, uint8_t FontSize, char *format, ...)
{
	...
	va_end(arg);							//结束变量arg
	OLED_ShowString2(X, Y, String, FontSize);//OLED显示字符数组（字符串）
}
